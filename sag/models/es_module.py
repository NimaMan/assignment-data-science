...
class ESModule(Module):
    ...
    def set_model_flat_params(self, params):
        idx = 0
        for i, param in enumerate(self.parameters()):
            delta = np.product(self.model_shapes[i])
            block = params[idx : idx + delta]
    ...
    def set_model_block_params(self, params):
        idx = 0
        for i, param in enumerate(self.parameters()):

            block = params[i]
            layer_data = torch.from_numpy(block).float()
            param.data = layer_data

        return self

    def set_model_params(self, params, flat_params=True):
        model_shapes = self.model_shapes
        if flat_params:
            return self.set_model_flat_params(params)
        else:
            pass
